...
where each tab opens a separate panel). Four panels are available from the **View**
menu: Prog, BoatSim, Seatalk, and AIS. Each panel can be opened and closed independently;
...
## AIS Window (winAIS.pm)

The AIS window is a live, plotter-style browser of the virtual AIS targets
("vboats") generated by the boat simulator.  It is activated by `B_AIS=1` on open
and `B_AIS=0` on close.

Each `BINARY_TYPE_AIS` packet is aggregated -- it carries only whichever vboat(s)
actually transmitted in that burst (usually one) -- so the window accumulates
contacts keyed by **MMSI**, updates each as its reports arrive, and ages a contact
out after it has not been heard for a timeout (a vboat that has cycled out of
range).  A vboat on a `COLLIDE` course is shown in **red**.

**Columns:**

| Column   | Description                                                    |
|----------|----------------------------------------------------------------|
| MMSI     | Maritime Mobile Service Identity (the contact key)             |
| Name     | Vessel name                                                    |
| Type     | AIS ship-and-cargo type code                                   |
| Lat/Lon  | Position (degrees-minutes)                                     |
| COG/SOG  | Course and speed over ground                                   |
| Rng(NM)  | Range from the simulated boat, nearest first                   |
| Brg      | True bearing from the simulated boat                           |
| Msg      | The AIS message just received (POS / NAME / STAT)              |
| Age      | Seconds since this contact was last heard                      |

The list is a standard `Wx::ListCtrl` (report style) sized to fill the window on
open (via `onActivate()`) and on every resize.
